...
def challenge1(question):
	main_logic()
	#question = input("Q: ")

	for i in range(1,len(data["module"][2])):	
		similarity = similar.cos_sim(question,data["module"][2][i]["subtopic_name"])
		print(similarity)
		if(similarity > 0.2):
			print(data["module"][2][i]["subtopic_name"])
			ans1 = summarize.getSummarization(data["module"][2][i]["content"],5)
			print(ans1)
			return ans1
		return 0
...
def challenge2():
	...
	for i in range(1,len(data["module"][2])):	
		...
		#print("\n")
		answer = summarize.getSummarization(data["module"][2][i]["content"],1)
		#print(answer)
